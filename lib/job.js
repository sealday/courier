...
class Job extends EventEmitter {
  ...
  /**
   * 转换成对象形式
   * @return {{id: *, data: *, createdAt: (number|*), failedAt: number, updatedAt: (number|*), promotedAt: number, progress: Job.progress, error: string, attempts: number, maxAttempts: number, delay: number, priority: (number|*), ttl: (number|*), type: *, state: string}}
   */
  toJSON() {
    return {
      id: this.id,
   ...
   */
  doProgress(complete, total, data) {
    const client = this.redis.client;
    const id = this.id;
    const d = data || '';
  ...
  remove(fn) {
    ...
    client.multi()
      ...
      .del(`job:${this.id}`)
      .exec(fn);
    client.publish(`events:${this.id}`, JSON.stringify({
      id: this.id,
      name: 'remove',
    }));
  }
