...
class Courier extends EventEmitter {
    ...
    setInterval(() => {
      warlock.lock('locks:monitor', 2000, (err, unlock) => {
        ...
        if (typeof unlock === 'function') {
          const _unlock = _.after(3, unlock);
          this._promote(_unlock);
          this._ttlCheck(_unlock);
          this._restartFailed(_unlock);
        } else {
  ...
  post(type, data, opts) {
    ...
    job.delay = (options.delay || 0) * 1000;
    job.ttl = options.ttl || 60 * 60;
    const toState = options.delay ? 'delayed' : 'ready';

    client.incr('id', (err, id) => {
      if (err) return this.emit('error', err);

      job.init(id, pubsub, this._redis);

      job.requeue('', toState, err => {
        if (err) return this.emit('error', err);
      });
    });
    return job;
  ...
  process(type, fn) {
    ...
    blockingClient.brpoplpush(`jobs:${type}:ready`, `jobs:${type}:active`, 1, (err, id) => {
      ...
      client.hgetall(`job:${id}`, (err, rawJob) => {
        let job = new Job(rawJob.type, rawJob.data);
        _.assign(job, rawJob);
        job.redis = this._redis;
        if (err) return this.emit('error', err);
        fn(job, err => {
...
exports.createCourier = options => courier
  ? courier
  : new Courier(options);
